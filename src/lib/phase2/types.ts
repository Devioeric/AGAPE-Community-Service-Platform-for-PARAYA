...
export interface BeneficiaryEvidenceOptionDTO { id: string; cycleId: string; cycleName: string; barangayId: string; generatedAt: string; asOfDate: string; sampleMethod: string; approvedHouseholds: number; approvedResidents: number; coveragePercent: number | null; sourceKind: "approved_sample"; }
export interface BeneficiaryPlanningResultDTO { schema: "agape.beneficiary-estimate.v2"; categoryCode: string; predicateVersion: string; target: { barangayId: string; sitioId: string | null; scope: "barangay" }; evidenceSnapshotId: string; calculatedCount: number | null; suppressed: boolean; suppressedLabel: string | null; source: Record<string, unknown>; sample: Record<string, unknown>; quality: Record<string, unknown>; asOfDate: string; }
