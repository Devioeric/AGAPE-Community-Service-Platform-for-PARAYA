...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params; const { error } = await auth.supabase.rpc("phase1_void_impact_record", { p_entity_type: "qualitative", p_entity_id: id, p_reason: parsed.data.reason });
