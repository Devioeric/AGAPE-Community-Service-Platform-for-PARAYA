...
SELECT is((SELECT count(*) FROM pg_policies WHERE schemaname='public' AND (qual ILIKE '%role = ''admin''%' OR with_check ILIKE '%role = ''admin''%')),0::bigint,'policies do not grant Admin operational roaming');
