...
export async function POST(request: Request) {
  ...
  const workbook = form.get("workbook"); const programsCsv = form.get("programs_csv"); const sdgsCsv = form.get("sdgs_csv");
  try {
    let parsed;
    if (workbook instanceof File) {
      ...
      const combinedSize = programsCsv.size + (sdgsCsv instanceof File ? sdgsCsv.size : 0);
    ...
    const { data, error } = await auth.supabase.rpc("phase2_stage_historical_import", { p_file_hash: parsed.fileHash,
      p_template_version: HISTORICAL_TEMPLATE_VERSION, p_rows: rows, p_replaces_batch_id: String(form.get("replaces_batch_id") ?? "") || null });
