...
| F — Synthetic fixtures | in_progress | Phase 1 and Phase 2 seeds are split at the reviewed scope boundary. The Phase 1 fixture executed successfully with 20/20 integrity assertions and finishes with profiling off. The Phase 2 supplement remains unexecuted. | Use the Phase 1 fixture for Packet 10 behavioral suites, then validate the Phase 2 supplement in the full-chain gate. |
| G — Executable security suites | in_progress | Phase 1 catalog pgTAP executes successfully with 12/12 assertions, and the reviewed fixture executes with 20/20 assertions. Raw authenticated JWT/PostgREST, malicious RPC, concurrency, and behavioral Storage runners remain to be completed and executed. | Implement and execute the remaining Packet 10 suites without treating source-level registries as runtime proof. |
...
| 10 — Phase 1 technical gate | in_progress | Deterministic replay, catalog pgTAP, and seeded fixture integrity are green. The mandatory behavioral suites and final synthetic/rollback reconciliation have not yet executed. | Build the executable scenario runner from the reviewed registry, run every mandatory case with zero skips, and retain all outputs outside Git as non-approved local artifacts pending independent review. |
