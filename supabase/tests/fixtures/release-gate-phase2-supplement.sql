...
INSERT INTO public.partnership_documents(id,partner_id,term_id,document_type,original_name,storage_path,sha256,mime_type,size_bytes,scan_status,uploaded_by) VALUES
 ('f3130000-0000-4000-8000-000000000001','f3100000-0000-4000-8000-000000000001','f3120000-0000-4000-8000-000000000001','agreement','synthetic-agreement.pdf','synthetic/partners/f3100000-0000-4000-8000-000000000001/agreement.pdf',repeat('3',64),'application/pdf',128,'quarantined','f2200000-0000-4000-8000-000000000003') ON CONFLICT(id) DO NOTHING;
...
INSERT INTO public.historical_program_import_batches(id,file_hash,template_version,status,row_count,error_count,created_by) VALUES
 ('f3230000-0000-4000-8000-000000000001',repeat('5',64),'phase2.synthetic.v1','ready',1,0,'f2200000-0000-4000-8000-000000000004') ON CONFLICT(id) DO NOTHING;
...
INSERT INTO public.community_needs(id,barangay_id,submitted_by,category,title,description,priority,affected_count,sitio,approval_status,approved_by,approved_at) VALUES
 ('f3300000-0000-4000-8000-000000000001','f2100000-0000-4000-8000-000000000001','f2200000-0000-4000-8000-000000000007','education','Synthetic learning need','Synthetic aggregate planning need.','high',20,'Synthetic Sitio North','approved','f2200000-0000-4000-8000-000000000006',now()) ON CONFLICT(id) DO NOTHING;
INSERT INTO public.project_proposals(id,title,rationale,objectives,target_beneficiaries,expected_beneficiary_count,barangay_id,start_date,end_date,budget,status,created_by,finance_clearance) VALUES
...
INSERT INTO public.proposal_beneficiary_categories(code,label,predicate_version,predicate_definition,created_by) VALUES('synthetic_households','Synthetic Households','synthetic-v1','{"kind":"aggregate_only"}','f2200000-0000-4000-8000-000000000002') ON CONFLICT(code) DO NOTHING;
...
INSERT INTO public.proposal_budget_revisions(id,proposal_id,revision_number,status,cash_total,in_kind_total,canonical_hash,row_version,created_by,submitted_by,submitted_at,cleared_by,cleared_at,frozen_snapshot) VALUES
 ('f3380000-0000-4000-8000-000000000001','f3310000-0000-4000-8000-000000000001',1,'cleared',1000,0,repeat('6',64),1,'f2200000-0000-4000-8000-000000000003','f2200000-0000-4000-8000-000000000003',now(),'f2200000-0000-4000-8000-000000000005',now(),'{"cashTotal":"1000.00"}') ON CONFLICT(id) DO NOTHING;
...
INSERT INTO public.program_financial_documents(id,program_id,document_type,original_name,storage_path,sha256,mime_type,size_bytes,scan_status,uploaded_by) VALUES
 ('f3430000-0000-4000-8000-000000000001','f3320000-0000-4000-8000-000000000001','receipt','synthetic-receipt.pdf','synthetic/programs/f3320000-0000-4000-8000-000000000001/receipt.pdf',repeat('8',64),'application/pdf',128,'risk_accepted','f2200000-0000-4000-8000-000000000003') ON CONFLICT(id) DO NOTHING;
