...
| G — Executable security suites | in_progress | Phase 1 executable definitions exist and the canonical target chain now replays. Authenticated behavioral execution remains pending Packet 9 database corrections and candidate-mode test orchestration. | Implement Packet 9, then run the Phase 1 database/security suites against the disposable candidate chain. |
