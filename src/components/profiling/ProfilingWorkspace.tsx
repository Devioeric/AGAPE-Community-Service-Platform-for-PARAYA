...
import { Download, Loader2, RefreshCw, ShieldCheck, Upload } from "lucide-react";
...
type Cycle = { id: string; barangay_id: string; barangay_name?: string; name: string; status: string; sample_method: string; target_households: number; approved_households: number; row_version: number; collection_starts_on?: string | null; collection_ends_on?: string | null; collection_end?: string | null; captain_endorsed_at: string | null };
...
function ageAtDate(birthDate: string, asOfDate?: string | null): number {
...
export function ProfilingWorkspace({ audience, role }: { audience: "paraya" | "barangay"; role: string }) {
  ...
  const loadCycles = useCallback(async () => {
    ...
    try {
      ...
      setCycles((cycleRows ?? []).map((cycle: Cycle) => ({ ...cycle, collection_end: cycle.collection_ends_on }))); setNotices(noticeRows ?? []);
  ...
  function residentIsMinor(resident: ResidentDraft) {
    return resident.birth_date ? ageAtDate(resident.birth_date, selectedCycle?.collection_starts_on) < 18 : resident.estimated_age < 18;
  }
  ...
  async function savePackage(submit: boolean) {
    ...
    try {
      ...
      if (submit) await requestJson(`/api/profiling/submissions/${created.id}/submit`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ expected_version: 1 }) });
  ...
  async function decide(decision: "approve" | "return") {
    if (!detail) return;
    const reason = decision === "return" ? window.prompt("Required correction reason") : null;
  ...
  async function endorseCycle() {
    ...
    try {
      await requestJson(`/api/profiling/cycles/${selectedCycle.id}/endorse`, { method: "POST" });
  ...
  return <div className="space-y-6">
    ...
    {aggregate && <Card><CardContent className="p-4"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="flex items-center gap-2 font-semibold"><ShieldCheck className="h-5 w-5 text-success" />Approved sample aggregate</h2>{selectedCycle && <div className="flex gap-2"><a className={buttonVariants({ size: "sm", variant: "outline" })} href={`/api/profiling/analytics/${selectedCycle.id}/export?format=csv`}>CSV</a><a className={buttonVariants({ size: "sm", variant: "outline" })} href={`/api/profiling/analytics/${selectedCycle.id}/export?format=xlsx`}>XLSX</a></div>}</div><pre className="mt-3 overflow-auto rounded bg-muted/30 p-3 text-xs">{JSON.stringify(aggregate, null, 2)}</pre></CardContent></Card>}
    ...
    {detail && <Card className="border-primary"><CardHeader><CardTitle className="text-base">Secretary detail review · {detail.household_code}</CardTitle></CardHeader><CardContent className="space-y-3"><pre className="max-h-80 overflow-auto rounded bg-muted/30 p-3 text-xs">{JSON.stringify({ household: detail.household_data, residents: detail.residents }, null, 2)}</pre><Textarea readOnly value="Approval confirms the household fields, every resident, consent basis, and roster relationship were reviewed." /><div className="flex justify-end gap-2"><Button variant="outline" onClick={() => setDetail(null)}>Close</Button><Button variant="outline" onClick={() => void decide("return")}>Return with reason</Button><Button onClick={() => void decide("approve")}>Approve reviewed package</Button></div></CardContent></Card>}

    {audience === "paraya" && role === "paraya_researcher" && <Card><CardHeader><CardTitle className="text-base">Researcher setup and correction controls</CardTitle></CardHeader><CardContent className="grid gap-2 text-sm md:grid-cols-3"><a className={buttonVariants({ variant: "outline" })} href="/officer/partnerships">Barangay prefixes and official sources</a><a className={buttonVariants({ variant: "outline" })} href="/api/profiling/runtime">Runtime readiness API</a><span className="rounded border p-2 text-muted-foreground">Lifecycle, duplicate, assignment, and sample-register actions are audited APIs and remain unavailable to Admin.</span></CardContent></Card>}
