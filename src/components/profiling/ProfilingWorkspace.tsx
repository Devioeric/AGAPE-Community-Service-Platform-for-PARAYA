...
type Detail = Submission & { sample_reference?: string; household: Record<string, unknown>; household_data?: Record<string, unknown>; residents: DetailResident[]; consents?: Array<{ id: string; subject_type: string; resident_id: string | null; status: string; privacy_notice_version: string; consented_by_name: string | null; guardian_relationship: string | null; effective_from: string; effective_to: string | null }>; previous?: { id: string; status: string; submission_version: number; household: Record<string, unknown>; residents: DetailResident[] } | null };
type ResidentDraft = {
  resident_id?: string;
...
export function ProfilingWorkspace({ audience, role }: { audience: "paraya" | "barangay"; role: string }) {
  ...
  const [aggregate, setAggregate] = useState<ProfilingAggregateDTO | null>(null);
  ...
  const loadCycle = useCallback(async () => {
    ...
    if (results[2].status === "fulfilled") setAggregate((results[2].value ?? null) as ProfilingAggregateDTO | null);
  }, [selectedCycle]);
  ...
  return <div className="space-y-6">
    ...
    <Card><CardContent className="flex flex-wrap items-center gap-3 p-4"><label className="text-sm font-medium" htmlFor="profiling-cycle-select">Cycle</label><select id="profiling-cycle-select" data-testid="profiling-cycle-select" className="h-10 min-w-72 rounded-md border bg-background px-3" value={cycleId} onChange={(event) => setCycleId(event.target.value)}>{cycles.map((cycle) => <option key={cycle.id} value={cycle.id}>{cycle.barangay_name ? `${cycle.barangay_name} — ` : ""}{cycle.name} · {cycle.status}</option>)}</select>{selectedCycle && <><Badge>{selectedCycle.status}</Badge><span className="text-sm text-muted-foreground">{selectedCycle.approved_households}/{selectedCycle.target_households} approved</span>{role === "paraya_researcher" && <Button size="sm" variant="outline" onClick={() => void transitionCycle()}>Advance cycle</Button>}</>}</CardContent></Card>

    {selectedNotice && canCollect && <Card data-testid="approved-privacy-notice"><CardHeader><CardTitle className="text-base">Approved privacy notice · {selectedNotice.version}</CardTitle></CardHeader><CardContent className="space-y-2 text-sm"><p className="whitespace-pre-wrap">{selectedNotice.notice_text}</p><p className="text-muted-foreground">Controller: {selectedNotice.controller_name} · Contact: {selectedNotice.privacy_contact}</p><p className="text-muted-foreground">Retention: {selectedNotice.retention_summary}</p></CardContent></Card>}
    ...
    {canCollect && selectedCycle?.status === "collecting" && <div className="grid gap-4 xl:grid-cols-2">
      <Card data-testid="assigned-sitio-sample-queue"><CardHeader><CardTitle className="text-base">Assigned-sitio sample queue</CardTitle></CardHeader><CardContent className="space-y-3"><ControlledSelect value={sitioId} onChange={setSitioId} options={sitios.map((item) => item.id)} />{sitios.length > 0 && <p className="text-xs text-muted-foreground">Selected: {sitios.find((item) => item.id === sitioId)?.name}</p>}<div className="max-h-52 space-y-2 overflow-auto">{samples.map((sample) => <button type="button" data-testid={`sample-unit-${sample.sample_reference}`} key={sample.id} className={`w-full rounded border p-3 text-left text-sm ${sampleReference === sample.sample_reference ? "border-primary" : ""}`} disabled={!['not_contacted','unavailable'].includes(sample.contact_outcome)} onClick={() => setSampleReference(sample.sample_reference)}><span className="font-mono">{sample.sample_reference}</span><span className="float-right text-muted-foreground">{sample.contact_outcome}</span></button>)}</div>{sampleReference && <div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => void recordNonParticipation("refused")}>Record refusal</Button><Button size="sm" variant="outline" onClick={() => void recordNonParticipation("unavailable")}>Record unavailable</Button><Button size="sm" variant="outline" onClick={() => void seedReprofile()}>Load prior approved roster</Button></div>}</CardContent></Card>
    ...
    {role === "barangay_captain" && selectedCycle?.status === "completed" && <Card data-testid="captain-endorsement"><CardContent className="flex items-center justify-between gap-3 p-4"><div><p className="font-medium">Captain endorsement</p><p className="text-xs text-muted-foreground">Endorsement is separate and does not change approved counts.</p></div><Button onClick={() => void endorseCycle()} disabled={Boolean(selectedCycle.captain_endorsed_at)}>{selectedCycle.captain_endorsed_at ? "Endorsed" : "Endorse completed cycle"}</Button></CardContent></Card>}
  </div>;
}
