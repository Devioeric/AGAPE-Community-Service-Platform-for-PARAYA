...
test("capability aliases and generated permission controls use the canonical matrix", () => {
  ...
  assert.equal(CAPABILITY_MODULE["ai.recommendation.configure"], "ai_assistance");
  ...
  assert.equal(hasCapability("paraya_director", {}, "ai.recommendation.configure"), true);
  assert.equal(hasCapability("paraya_researcher", {}, "ai.recommendation.configure"), false);
  assert.equal(hasCapability("paraya_director", { ai_assistance: false }, "ai.recommendation.configure"), false);
});
